...
def test_ticktick_connection():
    ...
    if not client_id or not client_secret:
        print(
            "❌ ERROR: TICKTICK_CLIENT_ID or TICKTICK_CLIENT_SECRET environment variables are not set."
        )
        print(
    ...
    try:
        ...
        for i, project in enumerate(projects, 1):
            print(
                f"  - {project.get('name', 'Unnamed project')} (ID: {project.get('id', 'No ID')})"
            )
        ...
        if projects:
            ...
            if "error" not in parent_task and "id" in parent_task:
                ...
                print(
                    f"✅ Created parent task: {parent_task.get('title')} (ID: {parent_task_id})"
                )
                ...
                if "error" not in subtask:
                    ...
                    print(
                        f"❌ Failed to create subtask: {subtask.get('error', 'Unknown error')}"
                    )
                    # Clean up parent task
                    client.delete_task(test_project_id, parent_task_id)
            else:
                print(
                    f"❌ Failed to create parent task: {parent_task.get('error', 'Unknown error')}"
                )
